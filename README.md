...
For `sc2v2`, you also need the SMACv2 map files for scenarios such as
`10gen_protoss`, `10gen_terran`, and `10gen_zerg`. Install those maps using the
SMACv2 repository instructions and place them in your StarCraft II `Maps`
directory in addition to the standard SMAC maps.
...
```shell
# For SMACv2
python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=protoss_5_vs_5
```
...
SMACv2 scenario definitions live under `src/config/envs/smacv2_configs`. The
`env_args.map_name` for `sc2v2` should be the scenario config name, not the raw
StarCraft map name.
...
```
 ...
 How To Operate It
  ...
  1. Install Python deps with bash install_dependencies.sh.
  2. Install SC2 and SMAC maps with bash install_sc2.sh.
  3. Install the SMACv2 map files for 10gen_protoss, 10gen_terran, and 10gen_zerg into your SC2 Maps directory. This is still a separate requirement; I documented it in README.md:149.
  4. Run a baseline SMACv2 job with:

  python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=protoss_5_vs_5

  5. Pick another prepared scenario by changing only the scenario name:

  python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=terran_10_vs_11
  python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=zerg_20_vs_23

  6. See the available prepared scenarios in src/config/envs/smacv2_configs.
  7. To create a new scenario, copy one of those YAML files, rename it, edit the inner env_args, then run with the new filename stem as env_args.map_name.
  8. To override nested scenario settings from the CLI, pass Sacred overrides. Because I changed the merge to deep-merge, nested overrides now work:

  python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=protoss_5_vs_5 env_args.capability_config.n_units=6 env_args.capability_config.n_enemies=8

  9. For list fields, replace the whole list, for example:

  python3 src/main.py --config=qmix --env-config=sc2v2 with env_args.map_name=protoss_5_vs_5 "env_args.capability_config.team_gen.unit_types=['stalker','zealot']" "env_args.capability_config.team_gen.weights=[0.5,0.5]"

  10. To change the actual underlying StarCraft map, edit the scenario YAML’s inner env_args.map_name. Do not try to use the outer env_args.map_name for that, because the outer one selects the scenario file.

  What you can manipulate fully through this wiring:

  - Algorithm config via --config=...
  - Scenario selection via env_args.map_name=<scenario_file_stem>
  - Nested SMACv2 capability settings via CLI or by editing scenario YAMLs
  - Team composition, weights, unit counts, start-position generators, observation settings, reward settings, step settings, and other wrapper-supported env_args
