...
def build_td_lambda_targets(rewards, terminated, mask, target_qs, n_agents, gamma, td_lambda, target_entropy=None):
    # Assumes  <target_qs > in B*T*A and <reward >, <terminated >, <mask > in (at least) B*T-1*1
    # target_entropy: optional [B, T, 1] entropy bonus (Soft-QMIX style).
    #                 When provided, α·H(π) is added to the bootstrap value
    #                 at each timestep, turning the return into an entropy-
    #                 regularised objective.  Pass None for standard hard-max.
    if target_entropy is None:
        target_entropy = target_qs.new_zeros(*target_qs.shape)
