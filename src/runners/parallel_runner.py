...
from envs import ensure_env_registered
...
class ParallelRunner:
    ...
    def __init__(self, args, logger):
        ...
        ensure_env_registered(self.args.env)
    ...
    def reset(self):
        ...
        for parent_conn in self.parent_conns:
            ...
            if "agent_types" in data:
                agent_types_data.append(data["agent_types"])

        if "agent_types" in self.batch.scheme:
            if len(agent_types_data) != self.batch_size:
                agent_types_data = [[0] * self.args.n_agents for _ in range(self.batch_size)]
            self.batch.update({"agent_types": agent_types_data})
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            env.reset()
            if hasattr(env, "get_agent_types"):
                agent_types = env.get_agent_types()
            else:
                agent_types = [0] * env.n_agents
            remote.send({
                "state": env.get_state(),
                "avail_actions": env.get_avail_actions(),
                "obs": env.get_obs(),
