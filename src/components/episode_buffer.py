...
def concat_episode_batches(batches):
    assert len(batches) > 0, "Need at least one batch to concatenate"
    if len(batches) == 1:
        return batches[0]

    reference = batches[0]
    new_data = reference._new_data_sn()
    for batch in batches[1:]:
        if batch.scheme != reference.scheme:
            raise ValueError("Cannot concatenate EpisodeBatches with different schemes")
        if batch.groups != reference.groups:
            raise ValueError("Cannot concatenate EpisodeBatches with different groups")
        if batch.max_seq_length != reference.max_seq_length:
            raise ValueError("Cannot concatenate EpisodeBatches with different max_seq_length")
        if batch.device != reference.device:
            raise ValueError("Cannot concatenate EpisodeBatches on different devices")

    for key in reference.data.transition_data:
        new_data.transition_data[key] = th.cat(
            [batch.data.transition_data[key] for batch in batches], dim=0
        )
    for key in reference.data.episode_data:
        new_data.episode_data[key] = th.cat(
            [batch.data.episode_data[key] for batch in batches], dim=0
        )
...
class ReplayBuffer(EpisodeBatch):
    ...
    def insert_episode_batch(self, ep_batch):
        if self.buffer_index + ep_batch.batch_size <= self.buffer_size:
            self._insert_episode_batch_at(ep_batch, self.buffer_index)
    ...
    def _insert_episode_batch_at(self, ep_batch, start_idx):
        end_idx = start_idx + ep_batch.batch_size
        ...
        self.update(ep_batch.data.episode_data,
                    slice(start_idx, end_idx))
        self.episodes_in_buffer = max(self.episodes_in_buffer, end_idx)
...
class ReservoirReplayBuffer(ReplayBuffer):
    ...
    def insert_episode_batch(self, ep_batch):
        for ep_idx in range(ep_batch.batch_size):
            single_episode = ep_batch[ep_idx:ep_idx + 1, :]
            if self.episodes_in_buffer < self.buffer_size:
                self._insert_episode_batch_at(single_episode, self.episodes_in_buffer)
            else:
                reservoir_idx = random.randint(0, self.episodes_seen)
                if reservoir_idx < self.buffer_size:
                    self._insert_episode_batch_at(single_episode, reservoir_idx)
            self.episodes_seen += 1

    def __repr__(self):
        return "ReservoirReplayBuffer. {}/{} episodes. Seen:{} Keys:{} Groups:{}".format(
